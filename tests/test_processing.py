...
@pytest.fixture
def test_data_simple() -> List[Dict]:
    """
...
@pytest.fixture
def test_data_duplicates() -> List[Dict]:
    """
...
@pytest.fixture
def test_data_mixed_states() -> List[Dict]:
    """
...
@pytest.fixture
def test_data_invalid_dates() -> List[Dict]:
    """
...
@pytest.mark.parametrize(
    "fixture_name, state_default, expected",
    [
        (
            "test_data_simple",
            "EXECUTED",
        ...
        ("test_data_mixed_states", "PENDING", [{"state": "PENDING", "date": "2023-01-03T12:00:00", "data": 3}]),
        ("test_data_empty", "EXECUTED", []),
    ],
)
def test_filter_by_state(fixture_name: str, state_default: str, expected: List[Dict], request: Any) -> None:
...
@pytest.mark.parametrize(
    "fixture_name, sort_increase, expected",
    [
        (
            "test_data_simple",
            True,
...
def test_sort_by_date(fixture_name: str, sort_increase: bool, expected: List[Dict], request: Any) -> None:
    """Тесты сортировки по дате с использованием фикстур."""
    input_list = request.getfixturevalue(fixture_name)
    assert sort_by_date(input_list, sort_increase) == expected


def test_sort_by_date_invalid_format(test_data_invalid_dates: List[Dict]) -> None:
    """Тест обработки некорректного формата даты."""
    with pytest.raises(ValueError):
        sort_by_date(test_data_invalid_dates)


def test_sort_by_date_empty(test_data_empty: List) -> None:
